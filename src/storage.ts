...
import { createDefaultPrizePools } from './data/defaultPools';
...
export const loadPrizePools = (): PrizePool[] => {
  if (typeof window === 'undefined') return createDefaultPrizePools();

  let stored: PrizePool[] = [];
  try {
  ...
  // The built-in roster is a fixed fixture: it is restored in roster order on every boot, and
  // whatever the user keeps alongside it is appended unchanged.
  const presets = createDefaultPrizePools();
  const presetIds = new Set(presets.map((pool) => pool.id));
  return [...presets, ...stored.filter((pool) => !presetIds.has(pool.id))];
};
