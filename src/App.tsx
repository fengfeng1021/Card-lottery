...
import { DrawRecord, PrizePool } from './types';
...
export default function App() {
  ...
  const openEditPool = (pool: PrizePool) => {
    if (isPresetPool(pool.id)) return; // Built-in pools are read-only.
    setEditingPool(pool);
    setIsPoolModalOpen(true);
  };
  ...
  const handleDeletePool = (poolId: string) => {
    if (isPresetPool(poolId)) return; // Built-in pools are always present.
