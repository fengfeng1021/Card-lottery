...
import { isSameTicket } from '../lib/tickets';
...
/**
 * Keeps one row per stub. Pasting a list that already holds a row, or that lists the same stub under
 * two spellings, must not put a second copy of that stub in the box.
 */
const withoutRepeatTickets = (candidates: PrizeItem[], existing: PrizeItem[]): PrizeItem[] =>
  candidates.filter((candidate) => {
    const name = candidate.name.trim().toLowerCase();
    return !existing.some(
      (item) =>
        isSameTicket(item.id, candidate.id) ||
...
export default function CreatePoolModal({
  ...
  const importItems = (text: string) => {
    ...
    const keptItems = items.filter((item) => item.name.trim());
    const addedItems = withoutRepeatTickets(importedItems, keptItems);
    ...
    setItems(keptItems.length ? [...keptItems, ...addedItems] : addedItems);
    setImportText('');
    setError('');
    focusItem(addedItems[0].id);
  };
